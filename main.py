...
class getBalance():
    ...
    def __init__(self, loginURL = "https://my.calpoly.edu/cas/login", url = "https://cardcenter.calpoly.edu/student/welcome.php", onTarget= "https://www.calpolydining.com/diningprograms/freshman/plusdollars/"):
    ...
    def returnBalance(self):
        ...
        result = self.session_requests.get(self.url)
    ...
    def onTarget():
        page = urllib.request.urlopen(self.onTarget)
        page = requests.get(self.onTarget)
        time.sleep(10)
    ...
    def sendEMail(self,title, message, emailUsername, emailPassword, emailPort, emailServer, emailTo):
        msg = MIMEMultipart()
        msg['From'] = emailUesrName  # Adds the pertinant values to msg dictionary
        msg['To'] = emailTo
        msg['Subject'] = title
        ...
        server.sendmail(emailUsername, emailTo, EMAIL_MESSAGE)
        server.quit()
    ...
    def sendSMS(message, smsAccountSID, smsAuthToken, smsSendNum, smsRecieveNum):

        client = TwilioRestClient(smsAccountSID, smsAuthToken)
    ...
    # Send PushBullet
    # ========================================================
    def sendPushBullet(title, message, pushBulletAPI):
    ...
    def sendIFTTT(message, iftttSecretKey, iftttEventName):

        report = {}
        report["value1"] = message
        url = "https://maker.ifttt.com/trigger/{}/with/key/{}".format(
            iftttEventName, iftttSecretKey)
    ...
    def sendMessage(self,title,message):
        debug("\nExpected Message\n================\n{}".format(message))
        if self.sendMethod == 1:  # 1 = Email
            self.sendEMail(title, message)
        elif self.sendMethod == 2:  # 2 = SMS
            self.sendSMS(message)
        elif self.sendMethod == 3:  # 3 = Twilio
            self.sendIFTTT(message)
        elif self.sendMethod == 4:
